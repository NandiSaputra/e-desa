...
<div x-data="wilayahForm()" x-init="loadProvinces()">
<div 
    x-show="openModal"
    x-transition
...
>
    ...
    <div class="bg-surface rounded-lg shadow-custom-lg w-full max-w-2xl max-h-screen overflow-y-auto">
        <div class="flex items-center justify-between p-6 border-b border-border-light">
            <h3 class="text-lg font-heading font-semibold text-text-primary">Tambah Kartu Keluarga Baru</h3>
            <button @click="openModal = false" class="text-text-secondary hover:text-primary">
                <i class="fas fa-times text-lg"></i>
            </button>
        </div>
        <form method="POST" action="{{ route('kartu_keluarga.store') }}" class="p-6 space-y-6">
            @csrf
            <!-- Form Fields -->
            <div class=" relative grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="no_kk" id="no_kk" class="form-input" placeholder="Nomor Kartu Keluarga" required />
                </div>
                @error('no_kk')
    ...
    @enderror
                <div>
                    ...
                    <input type="text" name="kepala_keluarga" class="form-input" placeholder="Nama Kepala Keluarga" required />
                </div>
            </div>
...
<input type="hidden" name="desa" :value="desaNama">
                ...
                <div>
                    ...
                    <input type="text" name="rt" class="form-input" placeholder="RT" required />
                </div>
                <div>
                    <label class="block text-sm font-medium text-text-primary mb-2">RW</label>
                    <input type="text" name="rw" class="form-input" placeholder="RW" required />
            ...
            <div> 
                ...
                <textarea class="form-input h-24" name="alamat" placeholder="Alamat Lengkap" required></textarea>
            </div>
             
        

            <div class="flex items-center justify-end space-x-3 pt-6 border-t border-border-light">
                <button type="button"  onclick="resetKeluargaForm()" @click="openModal = false" class="px-6 py-2 border border-border text-text-secondary rounded-lg hover:bg-gray-50 transition-colors">
                    Batal
                </button>
